...
import com.example.component.JobScheduleCreator;
import com.example.entity.SchedulerJobInfo;
import com.example.repo.SchedulerRepository;
...
public class SchedulerServiceImpl implements SchedulerService {
                ...
                try {
                    ...
                    if (!scheduler.checkExists(jobDetail.getKey())) {
                        ...
                        jobDetail = scheduleCreator.createJob(
                            ...
                            jobInfo.getDescription(), jobInfo.getJobDataMap());

                        if (jobInfo.getCronJob() && CronExpression
                            .isValidExpression(jobInfo.getCronExpression())) {
                            trigger = scheduleCreator
                                .createCronTrigger(jobInfo.getJobName(), new Date(),
                                    jobInfo.getCronExpression(),
                                    SimpleTrigger.MISFIRE_INSTRUCTION_FIRE_NOW);
                        } else {
                            trigger = scheduleCreator
                                .createSimpleTrigger(jobInfo.getJobName(), new Date(),
                        ...
                        log.info("### Job {} , {} Not loading at startup ",
    ...
    public void updateScheduleJob(SchedulerJobInfo jobInfo) {
        Trigger newTrigger;
        if (jobInfo.getCronJob()) {
            newTrigger = scheduleCreator
                .createCronTrigger(jobInfo.getJobName(), new Date(), jobInfo.getCronExpression(),
                    SimpleTrigger.MISFIRE_INSTRUCTION_FIRE_NOW);
        } else {
            newTrigger = scheduleCreator
                .createSimpleTrigger(jobInfo.getJobName(), new Date(), jobInfo.getRepeatTime(),
                    SimpleTrigger.MISFIRE_INSTRUCTION_FIRE_NOW);
        }
        try {
